"""
Auth middleware: resolves the incoming AIGate key to an Org + User identity.
...
async def resolve_identity(request: Request) -> ProxyIdentity:
    """
    Extract and validate the AIGate key from the Authorization header.
    ...
    if not raw_key.startswith("aip_"):
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid AIGate key format. Expected: aip_<org>_<key>",
        )
    ...
    if matched.upstream_key_encrypted:
        from aigate.crypto import decrypt
        try:
