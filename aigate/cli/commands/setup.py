"""aigate setup — configure AI coding tools to use the AIGate proxy."""
...
def _configure_openclaw(proxy_url: str, api_key: str) -> bool:
    """Configure OpenClaw to route through AIGate."""
...
def _configure_claude(proxy_url: str, api_key: str) -> bool:
    """Configure Claude Code to route through AIGate."""
...
def setup_interactive(
    ctx: typer.Context,
    url: str = typer.Option(None, "--url", "-u", help="AIGate proxy URL"),
    key: str = typer.Option(None, "--key", "-k", help="AIGate API key"),
):
    """Configure AI coding tools to use the AIGate proxy."""
    if ctx.invoked_subcommand is not None:
        return

    console.print(Panel(
        "[bold]Configure your AI coding tools to route through AIGate[/bold]",
        title="[accent]aigate setup[/accent]",
        border_style="accent",
    ))
    ...
    console.print()
    console.print(Panel(
        "[success]Done![/success] Your AI tools will now route through AIGate.\n"
...
def setup_openclaw(
    ...
    """Configure OpenClaw to route through AIGate, or reset to defaults."""
    if default:
        _reset_openclaw()
        return
    if not url:
    ...
    if not key:
        key = questionary.password("AIGate API key (aip_...):", style=_style).ask()
...
def setup_claude(
    ...
    """Configure Claude Code to route through AIGate, or reset to defaults."""
    if default:
        _reset_claude()
        return
    if not url:
...
def setup_status():
    ...
    console.print(Panel(
        "[bold]Current AIGate proxy configuration[/bold]",
        title="[accent]aigate setup status[/accent]",
        border_style="accent",
    ))
...
@app.command("reset")
def setup_reset():
    """Remove AIGate proxy configuration from all tools."""
    confirm = questionary.confirm(
        "Remove AIGate proxy config from all tools?",
        default=False,
        style=_style,
    ).ask()
    if not confirm:
