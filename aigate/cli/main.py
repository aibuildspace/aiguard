...
app = typer.Typer(
    name="aigate",
    help="AIGate — anti-virus for AI. Security middleware proxy for LLM APIs.",
    no_args_is_help=True,
)

from aigate.cli.commands.server import app as server_app
from aigate.cli.commands.org import app as org_app
from aigate.cli.commands.user import app as user_app
from aigate.cli.commands.shield import app as shield_app
from aigate.cli.commands.audit import app as audit_app
from aigate.cli.commands.setup import app as setup_app
from aigate.cli.commands.onboard import onboard as onboard_cmd
...
app.add_typer(setup_app, name="setup", help="Configure AI tools to use AIGate proxy")
...
def _start_server(
    *,
    ...
    console.print(f"[bold success]AIGate[/bold success] starting in {mode_label} on [accent]http://{host}:{port}[/accent]")
...
def start(
    ...
    """Start AIGate in DEV mode — full access, portal enabled."""
...
def startprod(
    ...
    """Start AIGate in PROD mode — locked-down, read-only admin API, no portal."""
