...
from aigate.observability.logging import configure_logging
...
async def lifespan(app: FastAPI):
    ...
    from aigate.proxy.forwarder import close_client
    await close_client()
    logger.info("AIGate shutdown complete")
...
def create_app() -> FastAPI:
    ...
    from aigate.api.endpoints.health import router as health_router
    app.include_router(health_router)

    # ── Management API ────────────────────────────────────────────────────
    if settings.admin_api_enabled:
        if is_prod:
            from aigate.api.router import build_prod_router
            app.include_router(build_prod_router(), prefix="/api/v1")
            logger.info("Admin API: PROD (read-only endpoints, key required)")
        else:
            from aigate.api.router import build_dev_router
    ...
    from aigate.proxy.router import router as proxy_router
