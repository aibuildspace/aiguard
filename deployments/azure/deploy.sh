...
if [[ -n "${OPENAI_FULL_KEY:-}" || -n "${ANTHROPIC_FULL_KEY:-}" ]]; then
    ...
    if [[ -n "${ANTHROPIC_FULL_KEY:-}" ]]; then
        echo ""
        echo "    # Claude Code"
        echo "    export ANTHROPIC_BASE_URL=http://$PUBLIC_IP:8080/anthropic"
        echo "    export ANTHROPIC_API_KEY=$ANTHROPIC_FULL_KEY"
    fi
    if [[ -n "${OPENAI_FULL_KEY:-}" ]]; then
        echo ""
        echo "    # OpenAI / Cursor"
        echo "    export OPENAI_BASE_URL=http://$PUBLIC_IP:8080/openai"
        echo "    export OPENAI_API_KEY=$OPENAI_FULL_KEY"
    fi
    ...
    echo ""
    echo "  Configure your AI tool:"
    if [[ "${onboard_provider:-}" == "anthropic" ]]; then
        echo ""
        echo "    # Claude Code"
        echo "    export ANTHROPIC_BASE_URL=http://$PUBLIC_IP:8080/anthropic"
        echo "    export ANTHROPIC_API_KEY=$FULL_KEY"
    fi
    if [[ "${onboard_provider:-}" == "openai" ]]; then
        echo ""
        echo "    # OpenAI / Cursor"
        echo "    export OPENAI_BASE_URL=http://$PUBLIC_IP:8080/openai"
        echo "    export OPENAI_API_KEY=$FULL_KEY"
    fi
else
    echo ""
