...
import typer
from aiguard.cli.theme import console, ACTION_COLORS, SEVERITY_COLORS, TUI_STYLE_DICT, HEX_ACCENT
...
def list_shields():
    ...
    from aiguard.shields.loader import load_shields
    ...
    async def _load_llm():
        from aiguard.db.engine import init_db, async_session_factory
    ...
    console.print("[muted]Use 'guard shield configure' to toggle shields interactively[/muted]")
...
def show_shield(
    ...
    from aiguard.config import settings
    from aiguard.shields.loader import load_shields, find_shield_file
    ...
    console.print("[muted]Use 'guard shield list' to see all shields including LLM shields.[/muted]")
    raise typer.Exit(1)
...
def _save_all_changes(items, originals, file_shields):
    ...
    if changed:
        ...
        console.print("[muted]Use 'guard shield reload' to apply to running server.[/muted]")
    else:
...
def edit_shield(
    ...
    \b
    Examples:
        guard shield edit prompt_injection --disable
        guard shield edit pii_detection --action sanitize --severity high
        guard shield edit content_policy --param blocked_keywords='["hack","exploit"]'
        guard shield edit prompt_injection -i
        guard shield edit my_llm_shield --action block --severity high
    """
    import questionary

    from aiguard.config import settings
    from aiguard.shields.loader import load_shields, find_shield_file, read_shield, write_shield
    ...
    console.print(f"\n[muted]Changes are live on next request. Use 'guard shield reload' to apply immediately.[/muted]")
...
def validate_shield(
    ...
    from aiguard.shields.loader import _load_shield, find_shield_file
