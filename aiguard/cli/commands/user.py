...
def create_key(
    ...
    async def _run():
        ...
        from aiguard.proxy.middleware import generate_api_key
        from sqlalchemy import select
...
def setup_user(
    ...
    Examples:
        guard user setup
        guard user setup --email alice@co.com --org acme --provider anthropic --tool claude
        guard user setup --no-interactive --email bob@co.com --org acme --provider openai --tool all
    """
    import questionary
...
def show_config(
    ...
    async def _run():
        ...
        async with async_session_factory() as session:
            ...
            if not keys:
                console.print(f"[warn]No API keys found for {email}. Run: guard user setup[/warn]")
                raise typer.Exit(1)
    ...
    console.print("[muted]Note: full key is not stored. Use 'guard user setup' to generate a new one.[/muted]")
