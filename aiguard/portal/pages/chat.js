...
function renderChatMessages() {
    ...
    container.innerHTML = chatMessages.map(m => {
        ...
        if (m.type === "assistant") {
            const outcome = m.proxyHeaders?.["x-aiguard-outcome"] || "";
    ...
    try {
        ...
        const res = await fetch(url, {
            ...
            headers: {
                ...
                ...(chatDisabledShields.size ? { "X-AIGuard-Disabled-Shields": [...chatDisabledShields].join(",") } : {}),
        ...
        for (const [k, v] of res.headers.entries()) {
            if (k.toLowerCase().startsWith("x-aiguard") || k.toLowerCase().startsWith("x-guard") || k.toLowerCase() === "traceparent") {
                proxyHeaders[k] = v;
            }
        }
        ...
        if (res.status === 403) {
            ...
            let findings = [];
            const findingsHeader = proxyHeaders["x-aiguard-findings"];
            if (findingsHeader) {
